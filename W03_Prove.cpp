...
//Code to evaluate a determine which space the player's piece will be place on.   
    switch(PlayerInput){
        ...
        default:
            cout<<"Sorry that space isn't on the board."<<endl;
    }
    ...
    //Code for if the location wants to put a piece has already been filled.
        ...
        cout<<"Sorry that place has already been filled. Please choose a different spot."<<endl;
        PlayerTurn();
    }
...
int GameOver(){
    for(int i=0;i<3;i++)
//C`rode that checks for row and column victories.
...
//Code that determine what should be printed to the screen 
//and under what conditions these messages will be shown.
    if (PlayerPiece == 'O' && draw == false){
        cout<<"Congratulation! You won Player 1";
    }
    else if (PlayerPiece == 'X' && draw == false){
